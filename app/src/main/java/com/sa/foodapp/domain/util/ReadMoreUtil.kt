package com.sa.foodapp.domain.util
...
open class BlackSpanClick(val context: Context) : ClickableSpan() {
    override fun updateDrawState(textPaint: TextPaint) {
        textPaint.isUnderlineText = false
...
private fun TextView.addClickablePartTextResizable(
    ...
    val click=object :BlackSpanClick(context){
        override fun onClick(widget: View) {
            if (viewMore) {
                setResizableText(fullText, maxLines, false, applyExtraHighlights)
            } else {
    ...
    builder.setSpan(click,0,shortenedText.length,0)
